...
prec = solve(cor_mat)
p0 = rep(0.1, d)
#momentum = c(-0.3980153048394741, 2.2483327571774736)
...
res = rcmg(n = n_iter, mean = mu, prec = prec, constraits = rep(1, d), t = 0.1, burnin = 0, p0 = p0, fixedMomentum = momentum, randomFlg = F, cppFlg = T, nutsFlg = T, debug_flg = F)

df = as.data.frame(fread(file="/Users/zhenyuzhang/beasttest/tempLogfiles/ZNUTS_20d.log", header = TRUE, quote ="")) 

assertthat::assert_that(max(df[2, 3:22] - res) < 1e-10)
```
...
samples = rcmg(n = n_hzz, mean = mu, prec = prec, constraits = rep(1, d), t = 10, burnin = 0, p0 = p0, randomFlg = T, cppFlg = T, nutsFlg = T, debug_flg = F)

true_samples <- t(MASS::mvrnorm(n_reject, mu, cov))
true_samples <- true_samples[, colSums(true_samples < 0) == 0]
dim(true_samples)

samples_tn <- TruncatedNormal::rtmvnorm(n = n_hzz, mu = mu, sigma = cov, lb = rep(0, d), ub = rep(Inf, d))
...
n_hzz = 1000
t0 = proc.time()
samples = rcmg(n = n_hzz, mean = mu, prec, constraits = rep(1, d), t = 10, burnin = 0, p0 = rep(0.1, d), cpp_flg = T)
t1 = proc.time()
cat(n_hzz, "hzz samples with dimension", d, "takes", t1[3] - t0[3], 'secs', '\n')

mcmc(samples, thin = 1) %>% effectiveSize() %>% summary()
plot(ts(samples[1, ]))
```
