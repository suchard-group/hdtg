...
profvis({samples_2 = rcmg(n = 3, mean = mu, prec, constraits = rep(1, d), t = 10, burnin = 0, p0 = rep(0.1, d), cpp_flg = T)})
...
engine0 = hzz::createEngineR(dimension = d, mean = rep(0, d), covMatrix = cor_mat, parameterSign = rep(1, d), flags = 128, info = 1, seed = 1)

first_bounce = hzz::getNextEvent(engine0$engine, position = position, velocity = velocity, action = action, logpdfGradient = - gradient, momentum = momentum)
...
t = hzz::hzz_cpp(engine0$engine, mean = rep(0, d), covMatrix = cor_mat, position = position, velocity = velocity, action = action, logpdfGradient = - gradient, momentum = momentum, time = 1)
